<template>
    <div class="home-container">
        <van-nav-bar class="van-nav-bar" fixed>
            <van-button class="search-btn" slot="title" tpye="info" size="small" round icon="search" to="/search">搜索</van-button>
        </van-nav-bar>

        <!-- 频道列表 -->
        <van-tabs v-model="active" animated swipeable class="channel-tabs" v-if="channel.name">
            <van-tab title="标签 1" v-for="channel in channels" :key="channel.id">
        ...
        <!-- 频道编辑弹出层 -->
        <van-popup v-model="ischannelshow" closeable position="bottom" :style="{ height: '100%' }" close-icon-position="top-left">
